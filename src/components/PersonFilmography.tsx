...
const CREW_ROLES = ['WRITER', 'PRODUCER', 'CINEMATOGRAPHER', 'COMPOSER', 'EDITOR']

function sortByYearDesc(films: FilmEntry[]): FilmEntry[] {
...
function FilmCard({ film, sparkColor }: { film: FilmEntry; sparkColor: string }) {
...
export function PersonFilmography({ filmography }: { filmography: FilmEntry[] }) {
  ...
  const directed = sortByYearDesc(filmography.filter((f) => f.roles?.includes('DIRECTOR')))
  const actedIn = sortByYearDesc(filmography.filter((f) => f.roles?.includes('ACTOR')))
  const crew = sortByYearDesc(
    filmography.filter(
      (f) => f.roles?.some((r) => CREW_ROLES.includes(r)) && !f.roles?.includes('DIRECTOR') && !f.roles?.includes('ACTOR')
    )
  )

  // If only one section exists, skip the section headings
  const sectionCount = [directed, actedIn, crew].filter((s) => s.length > 0).length
    ...
    <div className="mt-8">
      ...
      {directed.length > 0 && (
        <div className={sectionCount > 1 ? 'mb-6' : ''}>
          {sectionCount > 1 && (
            <h3 className="text-sm font-semibold text-cinema-gold uppercase tracking-wider mb-3">Directed</h3>
          )}
      ...
      {crew.length > 0 && (
        <div>
          {sectionCount > 1 && (
            <h3 className="text-sm font-semibold text-cinema-muted uppercase tracking-wider mb-3">Crew</h3>
          )}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {crew.map((film) => (
              <FilmCard key={`crew-${film.filmId}`} film={film} sparkColor="#888" />
            ))}
          </div>
        </div>
      )}
