...
// A single well-formed beat is enough — renderCoverSlide only needs dataPoints
// to populate FilmData, not to highlight anything (slide 1 has no beat pin).
const BEATS = [
  {
...
describe('renderCoverSlide', () => {
  it('throws DRAFT_NOT_FOUND when the draft is missing', async () => {
    ...
    const { renderCoverSlide } = await import('@/lib/carousel/render-cover-slide')
    await expect(renderCoverSlide({ draftId: DRAFT_ID })).rejects.toMatchObject({
  ...
  it('uses per-slide resolver when no override provided', async () => {
    ...
    mocks.prisma.carouselDraft.findUnique.mockResolvedValue({
      ...
      format: '4x5',
      backdropUrl: null,
      slideBackdropsJson: { '1': PER_SLIDE_URL },
    })
    const { renderCoverSlide } = await import('@/lib/carousel/render-cover-slide')
    await renderCoverSlide({ draftId: DRAFT_ID })
  ...
  it('calls composeSlide with slideNumber: 1', async () => {
    const { renderCoverSlide } = await import('@/lib/carousel/render-cover-slide')
    await renderCoverSlide({ draftId: DRAFT_ID })

    const arg = mocks.composeSlide.mock.calls[0][0]
    expect(arg.slideNumber).toBe(1)
  })
  ...
  it('exports RenderCoverSlideError with code/status/name', () => {
    const e = new RenderCoverSlideError('X', 'msg', 404)
    expect(e.message).toBe('msg')
    expect(e.code).toBe('X')
    expect(e.status).toBe(404)
    expect(e.name).toBe('RenderCoverSlideError')
  })
})
