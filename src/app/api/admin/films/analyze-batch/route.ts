...
export async function POST(request: Request) {
  ...
  if (!Array.isArray(filmIds) || filmIds.length === 0) {
    return Response.json({ error: 'filmIds must be a non-empty array' }, { status: 400 })
  }

  if (filmIds.length > 20) {
    return Response.json({ error: 'Maximum 20 films per batch' }, { status: 400 })
  }

  // Validate all IDs are strings
  if (!filmIds.every((id: unknown) => typeof id === 'string')) {
    return Response.json({ error: 'All filmIds must be strings' }, { status: 400 })
  }
